    ...
    function bindEvent() {
        ...
        $(document).on('mouseenter mouseleave', '#subIndexContent .content', function(e) {
            if (e.type === 'mouseenter') {
                $(this).find('img').addClass('active').next().show();
            } else {
                $(this).find('img').removeClass('active').next().hide();
            }
        ...
        $(document).on('input', '#title input', function(e) {
            ...
            $(_iframeWindow.document.head).find('title').text($(this).val() || '微网站');
        });
        //图片上传
        ...
        window.addEventListener('message', function(e) {
            ...
            if (_subContent.type === 0) {
                ...
                if (_subContent.data.hasOwnProperty('title')) {
                    $('.content .middle').show().find('#title').show().find('input').val(_subContent.data.title);
                } else {
                    $('#title').hide();
                }
                if (_subContent.data.hasOwnProperty('desc')) {
                    $('.content .middle').show().find('#abstract').show().find('textarea').val(_subContent.data.desc);
                } else {
                    $('#abstract').hide();
                }
                if (_subContent.data.hasOwnProperty('img')) {
                    $('.content .middle').show().find('#uploadImg').show().find('>span').text('建议上传图片大小为:' + _subContent.data.img.width + ' * ' + _subContent.data.img.height);
                } else {
                    $('#uploadImg').hide();
                }
                if (_subContent.data.hasOwnProperty('link')) {
                    ...
                    if (/\/subtemp\//.test(_subContent.data.link)) { //二级模板
                        _subStatus = 1;
                        $('#subpage > .link > i').removeClass('active').next().val('').prop('disabled', true);
                        $('#subpage > .type').find('i').addClass('active').parent().find('.first').addClass('active').parent().find('.second').text('重选模板').addClass('active');
                    } else if (/javascript/.test(_subContent.data.link) || $.trim(_subContent.data.link) === '') { //空跳转
                        _subStatus = 2;
                        $('#subpage > .link > i').removeClass('active').next().val('').prop('disabled', true);
                        $('#subpage > .type').find('i').addClass('active').parent().find('.first').removeClass('active').parent().find('.second').text('选择模板').addClass('active');
                    } else { //自定义跳转
                        _subStatus = 0;
                        $('#subpage > .type').find('.l').removeClass('active').last().text('选择模板');
                        $('#subpage > .link > i').addClass('active').next().val(_subContent.data.link).prop('disabled', false);
                    }
                } else {
                    $('#subpage').hide();
                }
